...
yages=(yFm-1)*1000;  %This is D14C, as opposed to an age - formerly -8033ln(yFm)


for ii=1:length(errFm)
    err(ii)=2*errFm(ii)*1000;
end
...
if nargin<4
    ylim14C=[-1000 0];ylim13C=[-30 -20];ylimCO2=[0 roundn(1.1*max(D(:,2)),1)];
end
if nargin<3
...
if sum(isnan(R(:,Fmcolumn)))==0     %If there are not any NaN's
    ...
    line([min(xlimit) max(xlimit)],[(WBD14C) (WBD14C)],'Color','r')
    line([min(xlimit) max(xlimit)],[(WBD14C+2*WBFmsigma*1000) (WBD14C+2*WBFmsigma*1000)],'Color','r','LineStyle','--')
    line([min(xlimit) max(xlimit)],[(WBD14C-2*WBFmsigma*1000) (WBD14C-2*WBFmsigma*1000)],'Color','r','LineStyle','--')
    text(min(T),1.05*(WBD14C),['Weig. Bulk \Delta^{14}C = ',...
        num2str(roundn((WBD14C),2)),' +/- ',...
        num2str(roundn(WBFmsigma*1000,1))],'Color','r');
else
