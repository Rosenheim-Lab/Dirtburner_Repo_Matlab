function [mC, sigmC, linvals_m]=LiveBlankDeterm(datafile,sigmas,analystindexfile)
%
%This function caclulates and graphically displays the amount of blank
...
%that consists of:
%Sample Mass (micrograms C) | Fraction modern | Fraction modern uncertainty
%|NOSAMS Reporting Date |Analysis Date Code | Name Code | Run Number |
%Split | Graphite Lot Number
...
%Load data
D=load(datafile);
D=sortrows(D,6);  %sorted on analyst number to accomodate legend for analyst plots.

%Plot the blank determinations on log-log space
figure(101)

loglog(D(:,1)/1000,D(:,2),'sk','MarkerFaceColor','b');
...
for n=1:length(D(:,1))
    mM(n)=(D(n,2)*D(n,1))/1; %Two terms cancel out because fm of graphite =
    %0, 1 is fm of modern material.  Equation 3 of Alvarez et al 2015,
    %Analytical Chemistry, supplemental materials
end
mC=mean(mM);
sigmC=std(mM);
Xes=order(xlim);
...
negs=ylim;
%Set number of lines to display, no more than 5
...
for z=1:length(xint)
    corrlines(z,3)=mC/1000*1/xint(z);
    if corrlines(z,3)<negs(1)
            corrlines(z,3)=NaN;
    end
    corrlines(z,2)=(mC-sigmC*sigmas)/1000/xint(z);
     if corrlines(z,2)<negs(1)
            corrlines(z,2)=NaN;
    end
    corrlines(z,4)=(mC+sigmC*sigmas)/1000/xint(z);
...
title('Modern Blank Analysis, Raw Scatterplot')
...
loglog(corrlines(:,1),corrlines(:,4),'-.r')
loglog(corrlines(:,1),corrlines(:,3),'-r','LineWidth',2)
title('Modern Blank by Analysis Date');
xlabel('Sample Size (mg C)')
ylabel('Fraction Modern')
