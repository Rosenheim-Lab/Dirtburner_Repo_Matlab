...
for ii=1:length(errFm)
    err(ii)=(-8033*log(yFm(ii)-errFm(ii))+8033*log(yFm(ii)+errFm(ii)));
end
...
if nargin<4
    ylim14C=[0 40000];ylim13C=[-30 -20];ylimCO2=[0 roundn(1.1*max(D(:,2)),1)];
end
if nargin<3
...
unevenbarerrcirc(xmark,yages2,0,err,sqfc,[0 0 0]);
ylabel('^{14}C y','FontSize',14);
...
if sum(isnan(R(:,Fmcolumn)))==0     %If there are not any NaN's
    ...
    WBFm=WBD14C/1000+1;
    WBFmsigma=sqrt(sum(E2))/n;
    WBAgeerr=(-8033*log(WBFm-2*WBFmsigma)-(-8033*log(WBFm+2*WBFmsigma)))/2;
    WBd13Cerr=sqrt(sum(R(:,Fmcolumn+3).^2));
    line([min(xlimit) max(xlimit)],[-8033*log(WBFm) -8033*log(WBFm)],'Color','r')
    line([min(xlimit) max(xlimit)],[-8033*log(WBFm+2*WBFmsigma) -8033*log(WBFm+2*WBFmsigma)],'Color','r','LineStyle','--')
    line([min(xlimit) max(xlimit)],[-8033*log(WBFm-2*WBFmsigma) -8033*log(WBFm-2*WBFmsigma)],'Color','r','LineStyle','--')
    text(min(T),1.05*-8033*log(WBFm),['Weig. Bulk Age = ',...
        num2str(roundn(-8033*log(WBFm),2)),' +/- ',...
        num2str(roundn(WBAgeerr,1))],'Color','r');
else
