function [D,Dn,Tmax,f, CompAge, Compd13C]=DecompDB2(filename,Int)
%************Decompose Dirt Burner Data************************************
%
%FOR USE ON RUNS BETWEEN 23MAY2012 AND 24MAY2012
%
%DecompDB(filename,Int) plots a thermograph from data in text file
%'filename' (4 columns: temperature, date number, time number, pCO2)
%using temperature intervals in Int.
%
%Outputs
%
%[D]=DecompDB(filename,Int) plots a thermograph from data in teh text file
%'filename' (4 columns: temperature, date number, time number, pCO2), temperature, 
...
%
%[D,Dn,Tmax]=DecompDB(filename,Int) plots a thermograph from data in the
%text file 'filename' (4 columns: temperature, date number, time number, pCO2), 
...
%temperaturevintervals when a shape run is performed).  D contains the data
...
%data in the text file 'filename' (4 columns: temperature, date number, 
%time number, pCO2), using the temperature intervals in Int (use [] 
...
%D = dlmread(filename, '\t');
D0=D(:,3)-min(D(:,3));
Dn=[D(:,4) D0./sum(D0)*length(D0)/10000;];  %normalized to measurement
...
plot(D(:,4),D0,'k','LineWidth',2) %plot zeroed pCO2
